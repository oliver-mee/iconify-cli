...
// iconify-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "ICONIFY_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  iconify-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  iconify-pp-cli recall "<question>" --agent
  iconify-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  iconify-pp-cli learnings list --agent
  iconify-pp-cli learnings list --query "<substring>"
  iconify-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  iconify-pp-cli learnings forget "<question>" --resource <id>
  iconify-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  iconify-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  iconify-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
