...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  iconify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  iconify-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  iconify-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"keywords", "sets"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  iconify-pp-cli workflow status

  # Show status as JSON
  iconify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("iconify-pp-cli")
			}
