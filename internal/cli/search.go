...
In local mode: searches locally synced data only.`,
  ...
  iconify-pp-cli search "error timeout"

  # Force local search only
  iconify-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  iconify-pp-cli search "status" --type icons --data-source local
  # JSON output for piping
  iconify-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'iconify-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
