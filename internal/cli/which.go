...
var whichIndex = []whichEntry{
	{Command: "index", Description: "Mirror icon sets and their icon names into local SQLite for offline search and analysis; --only scopes it to specific sets.", Group: "Local state that compounds", WhyItMatters: "Run this once before set-pick, swap, audit, or shadcn; it is what makes them answerable without the network."},
	{Command: "set-pick", Description: "Given the icon concepts you need, rank icon sets by how many of them each set actually covers.", Group: "Choose and stay consistent", WhyItMatters: "Reach for this before writing any UI that needs several icons, so every icon comes from one set at one stroke weight."},
	{Command: "shadcn", Description: "Turn Iconify icon names into shadcn.io registry slugs and optionally run the install.", Group: "Bridges to other tooling", WhyItMatters: "Use this when the project installs components from shadcn.io and you need the icon as a registry item rather than a raw SVG."},
	{Command: "kit", Description: "Render a whole directory of recoloured, uniformly sized SVGs from a manifest of icon names.", Group: "Batch rendering", WhyItMatters: "Use this when a deck, brand kit, or design system needs many icons at one colour and size rather than one ad-hoc icon."},
	{Command: "audit", Description: "Scan a codebase for Iconify references and report set spread, grid-height mismatches, palette mixing, and dead names.", Group: "Choose and stay consistent", WhyItMatters: "Use this on review to catch icons drifting across sets, or names that silently resolved through an alias."},
	{Command: "diff", Description: "Show which icons an upstream set added, removed, or aliased since your last sync.", Group: "Track upstream change", WhyItMatters: "Use this before upgrading an icon dependency, to see whether a name you rely on disappeared."},
	{Command: "swap", Description: "Map a list of icons from one set to another, classifying each as covered, renamed, or missing.", Group: "Choose and stay consistent", WhyItMatters: "Use this when standardising an existing codebase onto one icon set, to see the cost before committing."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  iconify-pp-cli which "stale tickets"
  iconify-pp-cli which "bottleneck"
  iconify-pp-cli which --limit 1 "send message"
  iconify-pp-cli which                                # list the full capability index`,
