...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"icon_set": {
			{Canonical: "mdi", Values: []string{"material design icons", "material icons", "materialdesign"}},
			{Canonical: "lucide", Values: []string{"lucide icons", "feather successor"}},
			{Canonical: "fa6-solid", Values: []string{"font awesome", "fontawesome", "fa solid", "font awesome solid"}},
			{Canonical: "fa6-brands", Values: []string{"font awesome brands", "brand icons", "logos"}},
			{Canonical: "simple-icons", Values: []string{"simpleicons", "brand logos", "company logos"}},
			{Canonical: "tabler", Values: []string{"tabler icons"}},
			{Canonical: "ph", Values: []string{"phosphor", "phosphor icons"}},
			{Canonical: "carbon", Values: []string{"ibm carbon", "carbon design"}},
			{Canonical: "material-symbols", Values: []string{"material symbols", "google symbols"}},
			{Canonical: "bi", Values: []string{"bootstrap icons", "bootstrap"}},
			{Canonical: "heroicons", Values: []string{"hero icons", "tailwind icons"}},
			{Canonical: "ri", Values: []string{"remix icon", "remixicon"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("iconify-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: iconify-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: iconify-pp-cli: learn init: %v\n", err)
		}
	})
}
