...
// spec-declared synonym pair is stamped through both normalizers: the
// entities.Config fold (read side) and the store query-pattern fold
// (write side).
func TestLearnConfig_SpecSynonymsRegisteredBothSides(t *testing.T) {
	cfg := newLearnConfig()
	...
	for variant, canonical := range pairs {
		famV := learn.QueryFamily(learn.Normalize(variant, cfg))
		famC := learn.QueryFamily(learn.Normalize(canonical, cfg))
		if famV != famC {
			t.Errorf("read side: spec synonym %q -> %q not folded (%q vs %q)", variant, canonical, famV, famC)
		}
		patV := store.NormalizeQuery(variant)
		patC := store.NormalizeQuery(canonical)
		if patV != patC {
			t.Errorf("write side: spec synonym %q -> %q not folded (%q vs %q)", variant, canonical, patV, patC)
		}
	}
}
