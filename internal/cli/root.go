...
type rootFlags struct {
	...
	// invocation. Mirrors the ICONIFY_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
Health check: run 'iconify-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("iconify-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'iconify-pp-cli profile list')")
	...
	rootCmd.AddCommand(newMetaCmd(flags))
	rootCmd.AddCommand(newSetsCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAuditCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDiffCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelKitCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSetPickCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelShadcnCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSwapCmd(flags))
