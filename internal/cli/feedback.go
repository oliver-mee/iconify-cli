...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ICONIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ICONIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/oliver-mee/iconify-cli/feedback")
...
When ` + "`ICONIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ICONIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  iconify-pp-cli feedback list
  iconify-pp-cli feedback list --limit 5
